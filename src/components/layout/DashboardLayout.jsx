...
export default function DashboardLayout({ children, role = "candidate" }) {
  ...
  const renderSidebarContent = () => (
    <>
      {/* Brand Header */}
      <div className="p-5 border-b border-slate-200/80 dark:border-white/[0.1] flex items-center justify-between">
        <Link to="/" onClick={() => setMobileSidebarOpen(false)} className="flex items-center gap-3 group">
        ...
        {mobileSidebarOpen && (
          <button
            ...
            className="lg:hidden p-1.5 rounded-lg border border-slate-200 dark:border-white/[0.1] text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"
          >
            <X className="w-4 h-4" />
          </button>
        )}
  ...
  return (
    <div className="min-h-screen bg-transparent text-[var(--text-primary)] flex overflow-hidden font-sans relative">
      ...
      {/* =========================================================
          DESKTOP SIDEBAR (Hidden on mobile, fixed 270px on desktop)
          ========================================================= */}
      <aside className="hidden lg:flex w-[270px] min-w-[270px] max-w-[270px] h-screen sticky top-0 flex-col bg-white/90 dark:bg-[#06101E]/90 backdrop-blur-2xl border-r border-slate-200/80 dark:border-white/[0.12] shadow-[4px_0_24px_rgba(0,0,0,0.03)] dark:shadow-[6px_0_30px_rgba(4,14,30,0.7)] shrink-0 z-30 select-none transition-colors duration-300">
        {renderSidebarContent()}
      </aside>

      {/* =========================================================
          MOBILE SLIDE-OUT DRAWER (Touch-friendly slide over)
          ========================================================= */}
      {mobileSidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          {/* Drawer Panel */}
          <div className="fixed inset-y-0 left-0 w-[280px] max-w-[85vw] flex flex-col bg-white dark:bg-[#06101E] border-r border-slate-200 dark:border-white/[0.12] shadow-2xl z-50 animate-in slide-in-from-left duration-200">
            {renderSidebarContent()}
          </div>
        </div>
      )}
      ...
      {/* =========================================================
          ...
          ========================================================= */}
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto relative z-10 w-full">
        
        {/* TOP HEADER */}
        <header className="h-14 bg-white/88 dark:bg-[#0e1823]/88 backdrop-blur-xl border-b border-slate-200/80 dark:border-white/[0.14] px-3 sm:px-6 lg:px-8 flex items-center justify-between gap-3 sticky top-0 z-20 shrink-0 shadow-sm transition-colors duration-300">
          
          {/* Left: Mobile Menu Toggle + Breadcrumb Title */}
          <div className="flex items-center gap-2 text-xs min-w-0">
            <button
              onClick={() => setMobileSidebarOpen(true)}
              className="lg:hidden p-1.5 rounded-lg border border-slate-200 dark:border-white/[0.12] bg-slate-100 dark:bg-white/[0.05] text-slate-800 dark:text-white shrink-0"
              title="Open menu"
            >
              <Menu className="w-4 h-4" />
            </button>
            <span className="text-slate-500 dark:text-[#94A3B8] font-mono uppercase font-bold hidden sm:inline">Portal</span>
            <ChevronRight className="w-3 h-3 text-slate-400 dark:text-[#94A3B8] hidden sm:inline" />
            <span className="font-extrabold text-slate-900 dark:text-white truncate text-xs sm:text-sm">
              {role === "government" 
                ? "Government Suite" 
                : role === "employer" 
                ? "Employer Suite" 
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <div className="hidden md:inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-cyan-50 dark:bg-[#22D3EE]/15 border border-cyan-200 dark:border-[#22D3EE]/40 text-cyan-800 dark:text-[#22D3EE] text-[10px] font-mono font-bold shadow-sm">
            ...
            <Link
              to="/login"
              className="hidden sm:flex px-3 py-1.5 rounded-lg border border-slate-200 dark:border-white/[0.14] bg-white/80 dark:bg-white/[0.08] hover:bg-slate-100 dark:hover:bg-white/[0.15] text-xs font-bold text-slate-800 dark:text-white transition-all items-center gap-1.5 shadow-sm"
            >
              <GraduationCap className="w-3.5 h-3.5 text-cyan-600 dark:text-[#22D3EE]" />
              <span>Switch</span>
            </Link>
          </div>
        </header>
